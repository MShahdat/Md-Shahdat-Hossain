  ...
  return (
    <div id='projects' className='bg-black text-white'>
      <div className='max-w-7xl px-4 py-12 mx-auto'>
        <div>
          <h1 className='text-3xl lg:text-4xl font-bold text-center text-primary tracking-wider bg-white/5 shadow-2xl shadow-primary w-fit mx-auto'>Recent Projects</h1>
        <div className='mt-3 w-[160px] lg:w-[200px] mx-auto h-[3px] rounded-2xl bg-primary'></div>
        </div>

        <div className='mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-10 gap-4 space-y-4'>
          {
            project.map((item, idx) => {
              return (
                <div
                  ...
                  <div className='group'>
                    <div className='relative hover:scale-102 duration-500 transform rounded shadow-2xl hover:shadow-primary'>
                      <img src={item.cover} className={`h-[300px] md:h-[320px] w-full object-cover `}></img>
                      <a href={item.live} target='_blank' className='cursor-pointer absolute top-4 right-4 bg-white text-primary w-8 h-8 rounded-full text-center flex items-center justify-center opacity-0 translate-y-1 group-hover:translate-y-0
